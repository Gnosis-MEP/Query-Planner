# Query Planner
Plans Query related data, such as query QoS criteria ranking, and (future) service function chain
...
```
$ ./query_planner/run.py
```

# Testing
Run the script `run_tests.sh`, it will run all tests defined in the **tests** directory.

Also, there's a python script at `./query_planner/send_msgs_test.py` to do some simple manual testing, by sending msgs to the service stream key.
...
But in order to make the automatic docker image build work, you'll need to set the `SIT_PYPI_USER` and `SIT_PYPI_PASS` variables in the Gitlab CI setting page: [Query Planner CI Setting Page](https://gitlab.insight-centre.org/sit/mps/felipe-phd/query-planner/settings/ci_cd). (Or make sure the project is set under a Gitlab group that has this setup for all projects in that group).
